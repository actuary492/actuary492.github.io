...
# Note the inputs: the effective rate (i), the term of the annuity (n), the frequency of payments (p), 
# and type that is a logical input (0: arrears or 1: advance) to tell the function if we are 
# calculating annuity in advance or annuity in arrears.
...
deferred_annuity(0.04,10, 3, 12, 0)
...
# Use Loans100 spreadsheet to find portfolio of Loans at certain years
...
# to be plugged into the function. It follows the same code as earlier, which is now adjusted a little.
...
# applicable element (note of the if function above).
...
# to change the year in the line ifelse(2021 > as.numeric(q[3])+as.numeric(q[5]), 0, df[2021-as.numeric(q[5])+1, 2])
# of 2021 into 2022, 2023, and further.
